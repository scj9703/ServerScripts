...
var LIGHT_DAMAGE = 0.35; // dmg value of light attack
var MEDIUM_DAMAGE = 0.5; // dmg value of medium attack
var HEAVY_DAMAGE = 0.75; // dmg value of heavy attack
var DEFENSE_PENETRATION = 0.3; // % of damage taken when blocking
...
function spawnFromPlayerOffset(npc, player)
{
    var xOffset = getRandomInt(2, 10);
    var zOffset = getRandomInt(0, 10);
    var rng = Math.random();
    if (rng > 0.5) xOffset = xOffset*-1;
    rng = Math.random();
    if (rng > 0.5) zOffset = zOffset*-1;
    npc.getWorld().spawnClone(player.x+xOffset, ARENA_CENTER[1], player.z+zOffset, NPC_TAB, CLONE_NAME);
}
...
function interact(e)
{
    ...
    npc.getTimers().forceStart(SPAWN_CLONE, 20, true);
}
...
function timer(e)
{
    ...
    switch(id)
    {
        ...
        case COMBO_SEVEN_ATTACK_2:
            ...
            break;
        case SPAWN_CLONE:
            spawnFromPlayerOffset(npc, target);
            break;
